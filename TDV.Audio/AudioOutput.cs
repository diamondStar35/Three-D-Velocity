...
namespace TDV.Audio
{
    public sealed class AudioOutput : IDisposable
    {
        ...
        public AudioSourceHandle CreateSource(string filePath, bool streamFromDisk = true)
        {
            var source = new AudioSourceHandle(this, filePath, streamFromDisk);
        ...
        public AudioSourceHandle CreateProceduralSource(ProceduralAudioCallback callback, uint channels = 1, uint sampleRate = 44100)
        {
            if (callback == null)
            ...
            var source = new AudioSourceHandle(this, sound, true, generator);
            if (_systemConfig.UseCurveDistanceScaler)
                source.ApplyCurveDistanceScaler(_systemConfig.CurveDistanceScaler);
            else
                source.SetDistanceModel(_systemConfig.DistanceModel, _systemConfig.MinDistance, _systemConfig.MaxDistance, _systemConfig.RollOff);
